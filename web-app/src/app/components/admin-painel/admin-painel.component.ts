...
export class AdminPainelComponent {
  ...
  ngOnInit(): void {
    this.userservice.gelAllUsers().subscribe(
    
      result => {this.listaUsers = result;
  ...
  delteUser(user: User):void{
    var confirmPassword
    confirmPassword = prompt(`Tornar o usuário ${user.nomeCompleto} administrador?\nEmail: ${user.email}\nCPF: ${user.cpf}\n\nSenha:`)
    this.passorwdTest = confirmPassword;

    this.userservice.deleteUser(user, this.passorwdTest).subscribe(
      result => {
        if(result.Sucess){
